"""
Modelo real de eficiencia energética de datacience — Modelo.py (versión
2026-08), envuelto en una función en vez de leer/escribir archivos.

CÓMO SE HIZO ESTE ARCHIVO (para que quede trazable):
Este archivo es una copia MECÁNICA del script original que entregó
datacience — no se retipeó ni se modificó ninguna línea de cálculo. Lo
único que cambió:
  1. Las 2 líneas que leían INPUT.json se reemplazaron por el parámetro
     `obs` de la función.
  2. Las 2 líneas que escribían OUTPUT.json se reemplazaron por `return result`.
  3. Todo el cuerpo entre esos dos puntos se indentó 4 espacios para vivir
     dentro de la función, sin cambiar el orden ni el contenido de ninguna
     línea.
  4. Se sacó el import de matplotlib (confirmado sin uso real: no hay
     ninguna llamada a plt. en todo el script).

Para verificar que este envoltorio no alteró nada: correr el script
original con un INPUT.json de prueba, y comparar su OUTPUT.json contra
predecir_eficiencia_real(ese mismo INPUT.json) — deben ser idénticos.
Ese es justo el test que vive en tests/test_modelo_real_datacience.py.
"""
import json
import numpy as np
import pandas as pd
...
def predecir_eficiencia_real(obs: dict) -> dict:
    ...
    # (import numpy as np / from scipy import stats: estaban acá en el
    # original, repetidos a mitad de script — se movieron arriba, a nivel
    # de módulo, porque repetirlos DENTRO de la función rompe el alcance de
    # 'np' para todo el resto de la función, un detalle de scoping de Python
    # que no existía cuando esto era un script suelto en vez de una función.)
