...
load_dotenv()
from src import calculos  # noqa: E402 — después de load_dotenv() a propósito: geo/llm leen el entorno al importarse
from src import geo  # noqa: E402
from src import llm as groq  # noqa: E402
from src import modelo as clasificador  # noqa: E402
from src import almacen  # noqa: E402


app = Flask(__name__)

# Se sincroniza con OCI (si está configurado) una sola vez, al arrancar el
# proceso — no en el primer request real ni en el primer /health, que debe
# seguir siendo rápido (ver docstring de health() más abajo).
clasificador.estado_oci()
...
def health():
    ...
    return jsonify({
        ...
        "modelo_entrenado": clasificador.modelo_disponible(),
        "oci": clasificador.estado_oci(),
    })
...
# ── Bloques horarios de uso (checklist, no un solo booleano) ────────────────
# Rangos discretos que el usuario puede marcar (uno o varios) para declarar en
# qué momentos del día usa mayoritariamente electricidad. "noche_pico" es el
# bloque que se considera horario pico para efectos de uso_horario_pico —
# 18-22h es una franja de horario pico habitual en varios tarifarios de la
# región; si el equipo de datacience define un rango distinto por país más
# adelante, este es el único lugar que hay que tocar.
RANGOS_HORARIOS = {
    "madrugada":       (0, 6,   "00:00–06:00 (madrugada)"),
    "manana_temprano": (6, 9,   "06:00–09:00 (mañana temprano)"),
    "manana":          (9, 13,  "09:00–13:00 (mañana)"),
    "mediodia":        (13, 15, "13:00–15:00 (mediodía)"),
    "tarde":           (15, 18, "15:00–18:00 (tarde)"),
    "noche_pico":      (18, 22, "18:00–22:00 (noche, horario pico típico)"),
    "noche_tardia":    (22, 24, "22:00–00:00 (noche tardía)"),
}


def _estadistica_horario_uso(rangos_seleccionados) -> dict:
    """Convierte el checklist de bloques horarios en una estadística real:
    cuántos bloques marcó, cuántas horas suman, y si alguno cae en horario
    pico — no es un booleano ciego, se puede auditar qué llevó a esa
    conclusión."""
    if not isinstance(rangos_seleccionados, list):
        rangos_seleccionados = []
    validos = [r for r in rangos_seleccionados if r in RANGOS_HORARIOS]
    horas_totales = sum(RANGOS_HORARIOS[r][1] - RANGOS_HORARIOS[r][0] for r in validos)
    return {
        "rangos_seleccionados": validos,
        "rangos_disponibles": {clave: etiqueta for clave, (_, __, etiqueta) in RANGOS_HORARIOS.items()},
        "cantidad_rangos_marcados": len(validos),
        "horas_totales_declaradas": horas_totales,
        "coincide_con_horario_pico": "noche_pico" in validos,
    }
...
def _sanitizar(data: dict) -> dict:
    ...
    return {
        ...
        # se preguntaba en ningún lado del wizard. Se acepta si viene
        # explícito (ej. quien llama directo a la API con el contrato del
        # PDF); si no viene, NO se deriva con un valor por defecto engañoso
        # — la ausencia real se distingue de un "no" explícito acá abajo,
        # después de armar el diccionario (ver _derivar_campos_contrato()).
        "uso_horario_pico":       data.get("uso_horario_pico"),
        "cantidad_equipos":       data.get("cantidad_equipos"),
        "horas_alto_consumo":     data.get("horas_alto_consumo"),
        # Checklist de bloques horarios — más preciso que un solo booleano.
        "rangos_horario_uso":     data.get("rangos_horario_uso") or [],
    }


def _derivar_campos_contrato(d: dict) -> dict:
    """C.1: completa uso_horario_pico / cantidad_equipos / horas_alto_consumo
    cuando no vinieron explícitos en el payload, a partir de lo que el
    asistente sí recoge — no se inventan, se calculan del mismo perfil de
    artefactos que ya usa el motor de cálculo.
    """
    if d["cantidad_equipos"] in (None, ""):
        d["cantidad_equipos"] = sum([
            d["aire_acondicionado"], d["calefaccion_electrica"], d["agua_caliente_electrica"],
            d["secarropas_electrico"], d["horno_electrico"], d["refrigerador"], d["freezer"], d["tv"],
        ])
    else:
        d["cantidad_equipos"] = int(d["cantidad_equipos"])

    if d["horas_alto_consumo"] in (None, ""):
        d["horas_alto_consumo"] = round(
            d["aire_acondicionado"] * 4.4 + d["calefaccion_electrica"] * 3.3
            + d["agua_caliente_electrica"] * 4.0, 1
        )
    else:
        d["horas_alto_consumo"] = float(d["horas_alto_consumo"])

    # Prioridad para uso_horario_pico: 1) el checklist de rangos horarios
    # (dato real que el usuario declaró), 2) el valor explícito del contrato
    # del PDF si vino directo, 3) la heurística por artefactos como último
    # recurso cuando no hay ninguno de los dos.
    estadistica_horario = _estadistica_horario_uso(d["rangos_horario_uso"])
    if estadistica_horario["cantidad_rangos_marcados"] > 0:
        d["uso_horario_pico"] = estadistica_horario["coincide_con_horario_pico"]
    elif d["uso_horario_pico"] in (None, ""):
        d["uso_horario_pico"] = bool(
            (d["aire_acondicionado"] or d["calefaccion_electrica"])
            and (d["habitantes_mayores"] + d["habitantes_menores"]) >= 2
        )
    else:
        d["uso_horario_pico"] = bool(d["uso_horario_pico"])

    return d


# _clasificar() (umbrales fijos 250/450) se eliminó de acá: la misma lógica
# de respaldo vive ahora en src/modelo.py::clasificar(), que la usa
...
def _calcular_analisis_energetico():
    ...
    resultado_clasificacion = clasificador.clasificar(d, consumo_kwh, fuente_consumo)
    ...
    resumen = {
        ...
        "horas_alto_consumo": d["horas_alto_consumo"],
        "estadistica_horario_uso": _estadistica_horario_uso(d["rangos_horario_uso"]),
    ...
    # solo que sin id de consulta.
    id_guardado = almacen.guardar(d, resumen)
    resumen["id"] = id_guardado

    return jsonify(resumen)

@app.route("/api/analisis-energetico/<id_analisis>", methods=["GET"])
@app.route("/analisis-energetico/<id_analisis>", methods=["GET"])
def obtener_analisis(id_analisis):
    """Fase D: endpoint de consulta de resultados que piden las bases."""
...
def _listar_analisis_energetico():
    """Fase D: historial — los últimos N análisis, más reciente primero."""
    limite = request.args.get("limite", default=20, type=int) or 20
    return jsonify({"analisis": almacen.listar(limite)})


@app.route("/api/analisis-energetico", methods=["GET", "POST"])
@app.route("/analisis-energetico", methods=["GET", "POST"])  # C.2 / H-2: alias del contrato del PDF
@limiter.limit("60 per hour")
def analisis_energetico():
    """Un solo Rule por ruta con ambos métodos — necesario para que el
    contrato OpenAPI (test_los_metodos_coinciden_con_los_registrados) pueda
    comparar 1:1 los métodos reales contra los documentados. Registrar GET y
    POST como decoradores separados crea DOS objetos Rule distintos para la
    misma ruta, cada uno exponiendo un solo método — el test compara cada uno
    contra la UNIÓN documentada (GET+POST) y siempre falla, aunque el código
    esté bien."""
    if request.method == "GET":
        return _listar_analisis_energetico()
    return _calcular_analisis_energetico()
