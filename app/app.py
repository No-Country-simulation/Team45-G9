...
def analisis_energetico():
    ...
    consumo_kwh = data.get('consumo_kwh', 0)
    horas_alto_consumo = data.get('horas_alto_consumo', 8)

    categoria = "Eficiente" if consumo_kwh < 200 else ("Moderado" if consumo_kwh < 400 else "Ineficiente")
    probabilidad = round(min(consumo_kwh / 500, 0.99), 2)
    costo_estimado = round(consumo_kwh * 0.75, 2)

    recomendaciones = []
    if data.get('uso_horario_pico'):
    ...
    if horas_alto_consumo > 6:
        recomendaciones.append("Distribuir las actividades de mayor consumo a lo largo del día")
